...
To solve this, I would create a for-loop and loop through the description elements and then once I reach a different group title, I change the group value and continue.

There are borders between each weather section in the excel file. These will come up as NaN in the data frame once I load it in. The solution is to loop through the entire dataframe and remote the NaNs.
...
```{r task2, include=TRUE, eval=TRUE}
...
```{r task2_data, include=TRUE, eval=TRUE}
knitr::kable(head(clean_air_cond_df, n = 10))
```
